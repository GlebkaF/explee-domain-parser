...
- **cron-job.org**: обработка каждую минуту (бесплатно)
- **Мгновенный триггер**: при нажатии "Run Agent"
- Vercel Cron настроен как резерв (раз в день)  

## Slice 4 — Агент v1: сбор HTML ✅
- Парсинг rawHtmlData с доменов (HTTPS/HTTP fallback)
- Сохранение `rawHtmlData` в базу данных
- Вывод данных под спойлером в таблице для завершенных агентов
- Кнопка копирования HTML в буфер обмена
- Отображение количества символов загруженного HTML  
